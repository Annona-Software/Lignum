 ...
 */
@Config(modid = Woodworking.MOD_ID, category = "")
@Mod.EventBusSubscriber(modid = Woodworking.MOD_ID)
@Config.LangKey("config." + Woodworking.MOD_ID)
//@SuppressWarnings("WeakerAccess")
public class ConfigFlasks {
    @Config.Comment("General settings")
    @Config.LangKey("config." + Woodworking.MOD_ID + ".general")
    public static final GeneralCFG GENERAL = new GeneralCFG();

    @SubscribeEvent
    public static void onConfigChangedEvent(ConfigChangedEvent.OnConfigChangedEvent event)
    {
        if (event.getModID().equals(Woodworking.MOD_ID))
        {
            LogManager.getLogger(Woodworking.MOD_ID).warn("Config changed");
            ConfigManager.sync(Woodworking.MOD_ID, Config.Type.INSTANCE);
        }
    }
